...
import com.restaurant.foody20.activity.Helper.TinyDB
...
class TinyDB(appContext: Context?) {
    ...
    private var DEFAULT_APP_IMAGEKIT_DIRECTORY: String? = null
    ...
    fun putImage(theFolder: String?, theImageName: String?, theBitmap: Bitmap?): String? {
        ...
        DEFAULT_APP_IMAGEKIT_DIRECTORY = theFolder
    ...
    private fun setupFullPath(imageName: String): String {
        val mFolder =
            File(Environment.getExternalStorageDirectory(), DEFAULT_APP_IMAGEKIT_DIRECTORY)
    ...
    fun getListObject(key: String?): ArrayList<PopularModel> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList = ArrayList<PopularModel>()
        for (jObjString in objStrings) {
            val player = gson.fromJson(jObjString, PopularModel::class.java)
    ...
    fun putListObject(key: String?, playerList: ArrayList<PopularModel?>) {
        checkForNullKey(key)
        val gson = Gson()
